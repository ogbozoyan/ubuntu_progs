...
uint32_t *ptr;
int char_my,char_enemy,temp,temp2=0;
...
void print(struct tron *tron,int x,int y,uint32_t enemy_color,char nap){
  ...
  switch(nap){
    case 'd':
      ...
      for(int i = 0;i<5;i++){
        if( (ptr[black_stick[i]*info.xres_virtual + c_x[8]] == enemy_color) ||
            (ptr[black_stick[i]*info.xres_virtual + c_x[8]] == 0xffffffff) ||
            (ptr[black_stick[i]*info.xres_virtual + c_x[8]] == tron->color) ) {
          end();
          }
      }
      for(int i = 0;i<9;i++){
    ...
    case 'w':
      ...
      for(int i = 0;i<5;i++){
        if( (ptr[c_y[8]*info.xres_virtual + black_stick[i]] == enemy_color) ||
            (ptr[c_y[8]*info.xres_virtual + black_stick[i]] == 0xffffffff) ||
            (ptr[c_y[8]*info.xres_virtual + black_stick[i]] == tron->color) ) {
        end();
          }
      }
      for(int i = 0;i<9;i++){
...
void* game(void *args){
  ...
  if(sum1>sum2){
   ...
   me.side = 0;
   me.prev_key = '.';
   enemy.x = info.xres - 10;
   enemy.y = info.yres - 30;
